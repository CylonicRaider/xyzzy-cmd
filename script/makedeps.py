...
def get_link_deps(f, cache=None, lcache=None, _stack=None):
    if cache is None:
        cache = {}
    if lcache is None:
        lcache = {}
...
def main():
    ...
    for p in proglist:
        ...
        for f in progs[p]:
            l.add(f)
            l.update(get_link_deps(f, cache, lcache))
        if not l: continue
        print (p + ': ' + ' '.join(l))
